...
export const campaign: Level[] = [
  ...
  level('ballast', 'Ballast', {
    width: 1440,
    height: 960,
    seconds: 50,
    par: 1,
    core: true,
    subtitle: 'ONE MEMORY CAN DO MORE THAN ONE JOB',
    objective: 'Deliver two crates to A and B, then hold C. Let your courier Echo open the way.',
    hint: 'Record a complete logistics run: carry the first crate onto A, pass its gate to collect the second crate, drop that on B, then circle below the tall partition and finish on C. Commit once. Follow the delivery Echo, collect the diamond core from the lower-right bay and exit through ABC. Do not take its crates.',
    spawn: point(3, 11),
    exit: point(33, 11),
    regions: [
      { ...wall(1, 1, 11, 22), name: 'FIRST DELIVERY' },
      { ...wall(13, 14, 10, 9), name: 'BALLAST BAY' },
      { ...wall(24, 1, 5, 22), name: 'CORE AISLE' },
    ],
    walls: [
      ...perimeter(36, 24),
      wall(12, 1, 1, 10),
    ...
    doors: [
      { id: 'A', ...wall(12, 11, 1, 2), signals: ['A'] },
      { id: 'ABC', ...wall(29, 11, 1, 2), signals: ['A', 'B', 'C'] },
    ],
    objects: [
      { id: 'first', ...point(4, 11), kind: 'crate' },
      { id: 'second', ...point(16, 16), kind: 'crate' },
      { id: 'payload', ...point(26, 20), kind: 'core' },
    ],
  }),
  level('crossfire', 'Crossfire', {
    width: 1600,
    height: 960,
    seconds: 45,
    par: 2,
    subtitle: 'BUILD A SAFE LINE OF FIRE',
    objective:
      'A protects the first crossing. B drops the second barrier and sentry shield. Clear the lane.',
    hint: 'Record A in the upper-left booth. In loop two, wait for A, cross the first laser, and record B in the lower middle. In the final loop, follow both Echoes, stop near x=1100 in the central lane, aim east and shoot the exposed sentry before crossing its range. The crate is movable cover; clever cargo routing can also reduce the number of Echoes you need.',
    spawn: point(3, 11),
    exit: point(37, 11),
    regions: [
      { ...wall(1, 1, 15, 22), name: 'RELAY A' },
      { ...wall(17, 1, 14, 22), name: 'RELAY B / FIRING LINE' },
      { ...wall(32, 1, 7, 22), name: 'SENTRY GALLERY' },
    ],
    walls: [...perimeter(40, 24), wall(10, 3, 1, 5), wall(22, 3, 1, 7)],
    plates: [
      { id: 'A', ...point(5, 4) },
      { id: 'B', ...point(23, 19) },
    ],
    lasers: [
      { id: 'WEST', ...wall(16, 1, 0.15, 22), signals: ['A'] },
      { id: 'EAST', ...wall(31, 1, 0.15, 22), signals: ['B'] },
    ],
    turrets: [{ id: 'GUARD', ...point(35, 11), range: 260, shield: 'B' }],
    objects: [{ id: 'cover', ...point(27, 18), kind: 'crate' }],
  }),
  level('homecoming', 'Homecoming', {
    ...
    par: 3,
    core: true,
    subtitle: 'THE EXIT IS WHERE YOU BEGAN',
    objective:
      'A opens the vault. B powers home. Retrieve the core and reunite with an Echo on SYNC.',
    hint: 'Record A in the northwest using the central opening in the west partition. Record B in the southwest through that same opening. Record a third Echo holding the origin pad. In your final loop, cross A into the eastern vault, circle the east end of its horizontal wall, retrieve the core, and retrace your route home. Extraction needs B and two bodies on SYNC at the origin.',
    spawn: point(23, 15),
    exit: point(23, 15),
    regions: [
      { ...wall(1, 1, 15, 11), name: 'NORTH STATION' },
      { ...wall(1, 20, 15, 11), name: 'SOUTH STATION' },
      { ...wall(17, 1, 15, 30), name: 'HOME / SYNC' },
      { ...wall(33, 1, 14, 30), name: 'REMOTE VAULT' },
    ],
    walls: [
      ...perimeter(48, 32),
      wall(16, 1, 1, 11),
    ...
    plates: [
      { id: 'A', ...point(5, 5) },
      { id: 'B', ...point(5, 25) },
      { id: 'SYNC', ...point(23, 15), need: 2 },
    ],
    doors: [{ id: 'A', ...wall(32, 14, 1, 3), signals: ['A'] }],
    objects: [{ id: 'home-core', ...point(42, 5), kind: 'core' }],
    exitSignals: ['B', 'SYNC'],
  }),
];
